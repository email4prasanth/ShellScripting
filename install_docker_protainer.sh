...
startInstall() 
{
    ...
    if [[ "$OS" != "1" ]]; then
        ...
        # (sudo apt update -y) > ~/docker-script-install.log 2>&1 &
    ...
    if [[ "$DCOMP" = [yY] ]]; then
        ...
        if [[ "$OS" == "1" ]]; then
            sudo curl -L "https://github.com/docker/compose/releases/download/1.29.2/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose >> ~/docker-script-install.log 2>&1
